(function (global, factory) {
    ...
    function stringifyEffect(eff) {
        ...
        if (/^scroll\s/.test(eff.name)) {
            return ((eff.name.replace(/^\w/, function (x) {
                return x.toUpperCase();
            })) + ";" + (eff.y1) + ";" + (eff.y2) + ";" + (eff.delay) + ";" + (eff.fadeAwayHeight));
        }
        return eff.name;
    }

    function stringifyDrawing(drawing) {
        return drawing.map(function (cmds) {
            return cmds.join(' ');
        }).join(' ');
    }
    ...
    function stringifyTag(tag) {
        ...
        var _ = tag[key];
        if (['pos', 'org', 'move', 'fad', 'fade'].some(function (ft) {
            return ft === key;
        })) {
            return ("\\" + key + "(" + _ + ")");
        }
    ...
    function stringify(ref) {
        ...
        return [
            ...
            '',
            '[V4+ Styles]',
            ("Format: " + (styles.format.join(', ')))].concat(styles.style.map(function (style) {
            return ("Style: " + (styles.format.map(function (fmt) {
                return style[fmt];
            }).join()));
        }),
            [''],
            ...
            (ref$1 = [])
                .concat.apply(ref$1, ['Comment', 'Dialogue'].map(function (type) {
                return (
                events[type.toLowerCase()].map(function (dia) {
                    return ({
                        start: dia.Start,
                        end: dia.End,
        ...
        for (var i = 0; i < parsed.length; i++) {
            ...
            for (var j$1 = 0; j$1 < tags.length; j$1++) {
                ...
                if (key && !~globalTags.indexOf(key)) {
                    ...
                    var compiledTag = compileTag(tag$1, key, {
                        start: start,
                        end: end,
                        c1: c1,
                        c2: c2,
    ...
    function decompileStyle(ref) {
        ...
        return ("Style: " + (stylesFormat.map(function (fmt) {
            return obj[fmt];
        }).join()));
    }
    ...
    var tagDecompiler = {
        ...
        move: function (_) {
            return ("move(" + ([_.x1, _.y1, _.x2, _.y2, _.t1, _.t2]) + ")");
        },
        fade: function (_) {
            return (
                _.type === 'fad'
        ...
        clip: function (_) {
            return ((_.inverse ? 'i' : '') + "clip(" + (_.dots
        ...
        t: function (arr) {
            return arr.map(function (_) {
                return ("t(" + ([_.t1, _.t2, _.accel, decompileTag(_.tag)]) + ")");
            }).join('\\');
        },
    };
    ...
    function decompileTag(tag) {
        return Object.keys(tag).map(function (key) {
            var fn = tagDecompiler[key] || (function (_) {
                return ("" + key + _);
            });
            return ("\\" + (fn(tag[key])));
        }).join('');
    ...
    function decompile(ref) {
        ...
        return [
            ...
            '',
            '[V4+ Styles]',
            ("Format: " + (stylesFormat.join(', ')))].concat(Object.keys(styles).map(function (name) {
